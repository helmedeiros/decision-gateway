...
Accepted — `internal/gateway` ships the Route + Router with longest-prefix selection; `internal/middleware` ships CorrelationID (read or mint X-Correlation-ID; echo on response) + AccessLog (one JSON line per request with the {time, level, msg, attrs={method, path, status, duration_ms, route, correlation_id}} shape that interops with traffic-gen/internal/jsonlog) + a RouteRecorder writer-side interface so the matched route flows from inner to outer middleware; `internal/proxy` wraps the router in an http.Handler that propagates the correlation ID to the backend, stamps the route on the writer, and forwards via per-route httputil.ReverseProxy; `internal/httpapi` ships Healthz + Readyz handlers shaped to match markup-svc ADR-0013 exactly; `cmd/decision-gateway` wires the four layers behind `--listen` + repeatable `--route=PREFIX=>BACKEND` + `--backend-timeout`. Boot emits a structured `gateway.boot` JSON event on stdout describing the listen address, the route table, and the backend timeout. The end-to-end test (`TestRunForwardsRequestsToBackend`) drives a real HTTP POST through the gateway to an httptest backend and asserts the correlation ID round-trips intact, the matched-route stamps land in the access log, and the boot event names the configured routes.
